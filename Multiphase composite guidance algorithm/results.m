...
t_terminate = 5;
options = odeset('Events', @(t, y) event_terminal(t, y), 'Refine', 1, 'maxstep', 0.01);
%% PPN, 2pPPN and BPPN
%N = 3;
alpha_P_df = 5*pi/6;
V_T = 0;
...
[t1,y1,B1] = ode45(@(t,y) BPPN_without_mu(t, y, V_P, V_T, alpha_P_df, alpha_P0, theta_0, V_theta_0, R_0), t_span, y0, options);
[t2,y2,B2] = ode45(@(t,y) BPPN(t, y, V_P, V_T, alpha_P_df, alpha_P0, theta_0, V_theta_0, R_0), t_span, y0, options);
...
for i = 1:length(t2)
    [~, B2] = BPPN(t2(i), y2(i,:), V_P, V_T, alpha_P_df, alpha_P0, theta_0, V_theta_0, R_0);
    B_2(i) = B2;
end
% length(B_1)
% length(B_2)
...
% 
theta_dot1 = y1(:, 3)./y1(:, 1);
theta_dot2 = y2(:, 3)./y2(:, 1);
figure;
td1 = plot(t1, (180/pi).*theta_dot1);
hold on;
td2 = plot(t2, (180/pi).*theta_dot2);
legend([td1, td2], 'Without Look-Angle Constraint', 'With Look-Angle Constraint', 'Location', 'best')
xlabel('Time [s]');
ylabel('$\dot{\theta}$', 'Interpreter', 'latex');
title('$\dot{\theta}$ vs Time', 'Interpreter', 'latex');
grid on;
